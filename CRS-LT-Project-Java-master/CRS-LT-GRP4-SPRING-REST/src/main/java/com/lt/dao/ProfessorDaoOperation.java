...
/**
 * 
 * @author Group-4 Class to implement Professor Dao Operations
 *
 */
@Service
	...
	public List<Course> getCoursesByProfessor(int profId) throws SQLException {
		...
		try {
			...
			while (results.next()) {
				...
				course.setCatalogId(results.getInt("catalog_id"));
				course.setProfessorId(results.getInt("professor_id"));
	...
	public List<EnrolledStudent> getEnrolledStudents(int profId) throws SQLException {
		...
		try {
			...
			while (results.next()) {
				...
				enrolledStudents.add(new EnrolledStudent(results.getString("course_code"),
						results.getString("course_name"), results.getInt("student_id")));
			}
		} catch (SQLException e) {
			logger.error(e.getMessage());
		} finally {
